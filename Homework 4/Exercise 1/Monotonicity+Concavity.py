...
e) Iterations of the value function taking into account both concavity of the value function
and monotonicity of the decision rule"""
...
def Bellman_MonCon(M,V0):
   X = np.empty([dim,dim])
   argmax_i = np.empty(dim)
   for i in range(dim):
       for j in range(dim):
           t = int(argmax_i[i])
           if t+j<dim:
               X[i,t+j] = M[i,j+t]+beta*V0[t+j]
...
V_MonCon, g_MonCon = solution(Bellman_MonCon)

stop = timeit.default_timer()
time_MonCon = stop - start
print('Time - Monotonicity+Concavity: '+ str(time_MonCon))
